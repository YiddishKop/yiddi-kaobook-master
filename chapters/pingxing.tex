...
\marginnote{    面积推论1：$ \triangle{ABD} \cong \triangle{BDC} $}
...
\marginnote{    面积推论3：$ \triangle AEO\ \cong \ \triangle CFO $}
...
\marginnote{面积推论5：$ S_{1} =S_{2} +S_{3} =\frac{1}{2} S_{\pllg ABCD} $}
...
本节的重点在于:“212”---“两条对边，一个对称中心，两条对角线”，理解\ubi{对边平行}是平四的核心性质, 
...
\exercise{}如图，在平行四边形ABCD中，已知∠ODA=90°，AC=10cm ，BD=6cm ，则AD的长为（\ \ \ ）\\
\tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
\begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1]
%uncomment if require: \path (0,862); %set diagram left start at 0, and has height of 862
%Shape: Parallelogram [id:dp5579947545995634] 
...
% -------   
\marginnote{
    【探索】请你尝试利用下图坐标系，画出符合要求的平行四边形，并体会:中位线的证明、求平四对称中心、平四存在性。
}
\begin{marginfigure}
